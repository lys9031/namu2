...
class MainWindow : public MyWindow {
 public:
  MainWindow(int w, int h, const std::string &windowTitle, const std::string &shaders);
  virtual ~MainWindow();
  ...
  void loadTemplateImageFromFile(char *path, bool scale = true);
  void setTemplateImageVisibility(bool visible);
  bool getTemplateImageVisibility();
  bool hasTemplateImage();
  void loadBackgroundImageFromFile(char *path, bool scale = true);
  ...
  void setCallBack(CALLBACK f, void* user_data);
 ...
 private:
  void initOpenGL(const std::string &shaders);
  void destroyOpenGL();
