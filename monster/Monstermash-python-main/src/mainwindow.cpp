...
MainWindow::MainWindow(int w, int h, const std::string &windowTitle, const std::string &shaders)
...
void MainWindow::setCallBack(CALLBACK f, void* u_data) 
{
  g_callback = f;
  user_data = u_data;
}

void MainWindow::initOpenGL(const std::string &shaders) {
  // opengl
...
void MainWindow::keyPressEvent(const MyKeyEvent &keyEvent) {
  ...
  g_callback(keyEvent.key, user_data);
...
void MainWindow::loadTemplateImageFromFile(char *path, bool scale) {
  // Imguc tmp = Imguc::loadImage("/tmp/template.img", 3, 4);
  Imguc tmp = Imguc::loadImage(path, 3, 4);
...
void MainWindow::loadBackgroundImageFromFile(char *path, bool scale) {
