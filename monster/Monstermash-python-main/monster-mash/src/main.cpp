...
extern "C" {
    MainWindow* mainWindow_new(int w, int h, char* title, char* shaders)
    {
        std::cout << title << std::endl;
        return new MainWindow(w, h, title, shaders);
    }
 
    void mainWindow_runLoop(MainWindow *m)
    {
        m->runLoop();
    }

    void mainWindow_setCallBack(MainWindow *m, CALLBACK f, void* user_data)
    {
        std::cout << user_data << std::endl;
        m->setCallBack(f, user_data);
    }

    void mainWindow_changeManipulationMode(MainWindow *m, SubManipulationMode mode)
    {
      m->changeManipulationMode(mode);
    }

    void mainWindow_saveProject(MainWindow *m, char* path)
    {
      m->saveProject(path);
    }
    
    void mainWindow_openProject(MainWindow *m, char* path)
    {
      m->openProject(path);
    }

    void mainWindow_reset(MainWindow *m)
    {
      m->reset();
    }

    void mainWindow_loadTemplateImageFromFile(MainWindow *m, char* path)
    {
      m->loadTemplateImageFromFile(path, true);
    }

    void mainWindow_loadBackgroundImageFromFile(MainWindow *m, char* path)
    {
      m->loadBackgroundImageFromFile(path, true);
    }
}