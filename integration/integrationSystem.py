...
def initSetting(flag):
    ...
    if flag=='init':
        ...
        pathList = [path_sum + 'integration/csvFile']
...
def eleminateDuplicateFileName(path, inputFileName, outFileName):
    lines = set()
...
def result(input, dist_score, size_score, histogram_score, triplet_score):
    ...
    scoreList = {'distance': dist_score, 'size' : size_score , 'histogram' : histogram_score, 'triplet' : triplet_score}

    for i in range(len(csvList)):
        eleminateDuplicateFileName(path, input_csvList[i], csvList[i])
...
def main():
    ...
    for i in range(len(csvList)):
        print(input_csvList[i], csvList[i])
        eleminateDuplicateFileName(path, input_csvList[i], csvList[i])

    for i in range(len(csvList)):
        print("dd")
        print(csvList[i], scoreList)
        sccoring(csvList[i], scoreList)
